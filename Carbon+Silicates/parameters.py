...
#'''Making O and C amounts equal'''
#X_C_tot = Y_O_tot *ph.A_C
...
in integrate.py'''
t_array, n_array, T_array, L_array = np.genfromtxt("trajectories/%s.dat"%traj,unpack=True)
n_init = n_array[0]
T_cs_init = T_array[0]
tmin = t_array[0]
tmax = t_array[-1]
...
directory = '%s/Cx%i/dt%.1e/e%.0e'%(traj,Camt,dt_init,epsilon)
# For C eq O :
#directory = '%s/CxeqO/dt%.1e/e%.0e'%(traj,dt_init,epsilon)
