...
K_CO_H    = re.CO_H_dest(T_cs) # Separate from CO rd beacuse it depends on Y_H
...
# Because we need the abundance to get the destruction rate. 
K_CO_nth  = 0.
K_SiO_nth = 0.
...
#------------ Initial abundances ------------
# This is tricky because we don't yet know the SiO, OH, and CO abundances
Y_OH = 0.
Y_SiO = 0. # for this function
Y_CO  = ab.Y_CO_equil(par.Y_C_tot, par.Y_O_tot, Y_SiO, n, K_CO_ra, K_CO_rd)
Y_SiO = ab.Y_SiO_equil(par.Y_Si_tot, par.Y_O_tot, Y_CO, n, K_SiO_ra, K_SiO_rd)
Y_OH = 0.
X_OH = ph.A_OH*Y_OH
X_CO  = ph.A_CO*Y_CO
X_SiO = ph.A_SiO*Y_SiO
Y_C   = par.Y_C_tot - Y_CO
X_C   = ph.A_C*Y_C
Y_O   = par.Y_O_tot - Y_CO - Y_SiO
X_O   = ph.A_O*Y_O
Y_Si  = par.Y_Si_tot - Y_SiO
X_Si  = ph.A_Si*Y_Si
Y_Mg  = par.Y_Mg_tot
X_Mg  = ph.A_Mg*Y_Mg
Y_H   = par.Y_H_tot
...
# Can make a "fake" number fraction Y_Mg2SiO4 that just adds the number of Si, O or Mg atoms that are in Si dust
...
# first line: 
fractionfile.write("%.5e %.5e %.5e %.5e %.5e %.5e %.5e %.5e %.5e %.5e %.5e %i %.5e %.5e \n"%(t,par.dt_init,\
	X_CO,X_C,X_O,X_solidC,X_Mg,X_Si,X_SiO,X_Mg2SiO4,X_H,adap_flag,sat_Cg,lnsat_Sig))
densfile.write("%.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f \n"%(t,\
	n_CO,n_C,n_O,n_solidC,n_Mg,n_Si,n_SiO,n_Mg2SiO4,n_H,ncritical_Cg,ncritical_Sig))
ratesfile.write("%.5e %.5e  %.5e %.5e %.5e %.5e %.5e %.5e %.5e \n"%(K_CO_ra, \
	K_CO_th, K_CO_nth, K_SiO_ra, K_SiO_th, K_SiO_nth,K_CO_H,J_Cg,J_Sig))
...
while t < par.tmax - par.dt_init:
	...
	if sat_Cg > 1: 
		...
		# We need to keep this dt factor out so it can be changed with the adaptive time step!!
	...
	# Need to keep dt*s out so they can be adaptive!!!
	...
	# Y_solidC is the number fraction of carbon atoms that are locked in carbon clusters
	...
	# CO detroyed by thermal, nonthermal, or by H
	Y_CO = np.maximum(Y_CO + delta_YCO_tot,par.floor)
	X_CO = ph.A_CO*Y_CO
	n_CO = Y_CO * n
	...
	X_SiO = ph.A_SiO*Y_SiO
	n_SiO = Y_SiO * n
	# H goes into destroying CO
	#Y_H   = np.maximum(Y_H + delta_YH_tot, par.floor)
	#X_H   = ph.A_H*Y_H
	#n_H   = Y_H * n
	...
	f_SiO = Y_SiO/(Y_CO+Y_C+Y_O+Y_Si+Y_Mg+Y_SiO+par.Y_other)
...
np.savetxt("runs/%s/Cdust.txt"%par.directory, Cdust_array, '%.5e', delimiter='   ',\
	header="# Y_grains each step         sum of Y_grains         size ")         
